...
export const defaultConfig: LPConfig = {
  ...
  waitingListConfig: {
    title: "ウェイティングリストに登録する",
    description: "",
    buttonText: "無料でウェイティングリストに登録する",
...
export function getConfigFromEnv(): LPConfig {
  const targetAudience =
    (process.env.NEXT_PUBLIC_TARGET_AUDIENCE as TargetAudience) || "demand";
  ...
  if (targetAudience === "supply") {
    return {
      ...
      waitingListConfig: {
        ...
        integrationType:
          (process.env.NEXT_PUBLIC_INTEGRATION_TYPE as
            | "notion"
            | "spreadsheet"
            | "email") || "email",
